  ...
  createMiddleware(channel: string) {
    return async (originalHandler: Function) => {
      return async (event: any, ...args: any[]) => {
        const config = this.configs.get(channel) || this.defaultConfigs[channel] || this.defaultConfigs.default;
        const key = config.keyGenerator ? config.keyGenerator(event, ...args) : `${channel}:${event.sender.id}`;
        ...
        if (entry.count > config.maxRequests) {
          entry.blocked = true;
          console.warn(`🚫 Rate limit exceeded for ${channel} (${key}) - ${entry.count}/${config.maxRequests}`);
          throw new Error(`Rate limit exceeded. Try again in ${Math.ceil((entry.resetTime - now) / 1000)} seconds.`);
        }
...
export function withRateLimit(channel: string, handler: Function) {
  ...
  return (event: any, ...args: any[]) => {
    return middleware(handler)(event, ...args);
  };
}
