...
async function runTuningProcess(job: TuningJob): Promise<void> {
  try {
    ...
    try {
      ...
      await fs.unlink(finalModelfilePath)
    } catch (error) {
      console.error(`Failed to clean up temporary files for job ${job.id}:`, error)
    }
  } catch (error) {
 ...
 * Cancel a tuning job
 */
export async function cancelTuningJob(id: string): Promise<boolean> {
  const job = await getJob(id)
 ...
 */
export async function exportDataset(id: string): Promise<string> {
  const dataset = await getDataset(id)
  if (!dataset) {
