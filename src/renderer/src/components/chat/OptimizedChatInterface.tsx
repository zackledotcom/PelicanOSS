...
const OptimizedChatInterface: React.FC<OptimizedChatInterfaceProps> = ({
  ...
  const handleSendMessage = async (content: string, attachments?: File[]) => {
    ...
    console.log('Creating user message with timestamp:', userMessage.timestamp, 'Type:', typeof userMessage.timestamp)
    ...
    try {
      ...
      if (response.success && response.response) {
        const assistantMessage: Message = {
          ...
          type: 'assistant',
          content: response.response, // Changed from response.message to response.response
          timestamp: new Date(),
          model: selectedModel,
          responseTime: responseTimeMs
        }

        console.log('Creating assistant message with timestamp:', assistantMessage.timestamp, 'Type:', typeof assistantMessage.timestamp)
        ...
        try {
          await analytics.trackChatMessage({
            ...
            sessionId,
            prompt: content.trim(),
            response: response.response, // Changed from response.message to response.response
            responseTime: responseTimeMs,
            success: true
          })
        ...
        throw new Error(response.response || response.error || 'No response from AI model')
      }
    } catch (error) {
      ...
      console.log('Creating error message with timestamp:', errorMessage.timestamp, 'Type:', typeof errorMessage.timestamp)
  ...
  const handleExportChat = () => {
    ...
    a.href = url
    a.download = `pelicanos-chat-${new Date().toISOString().split('T')[0]}.md`
