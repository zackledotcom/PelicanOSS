...
import { Brain, CaretDown, Cpu, Lightning, Warning, CheckCircle } from 'phosphor-react'
...
export default function ModelSelector({
  ...
  const getStatusIcon = (status: Model['status']) => {
    switch (status) {
      ...
      case 'error':
        return <Warning className="w-3 h-3 text-red-500" />
      case 'updating':
        return (
    ...
    <Card className={cn('w-80', className)}>
      ...
      <CardContent className="space-y-4">
          ...
          <div className="p-3 rounded-md bg-muted/50">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                <div
                  className={cn('flex items-center', getPerformanceColor(activeModel.performance))}
                >
            ...
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              ...
              <Badge
                variant={activeModel.type === 'local' ? 'default' : 'secondary'}
                className="text-xs"
              >
                {activeModel.type}
              </Badge>
              ...
              {models.map((model) => (
                <SelectItem key={model.id} value={model.id} disabled={model.status === 'error'}>
                  <div className="flex items-center justify-between w-full">
                    <div className="flex items-center gap-2">
                      <div
                        className={cn('flex items-center', getPerformanceColor(model.performance))}
                      >
        ...
        <div className="grid grid-cols-3 gap-2 pt-2 border-t text-center">
          ...
          <div>
            <p className="text-sm font-semibold">
              {models.filter((m) => m.type === 'remote').length}
            </p>
