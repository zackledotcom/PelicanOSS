...
const MemoryHealthIndicator: React.FC<MemoryHealthIndicatorProps> = ({ 
  ...
  const calculateMemoryHealth = (store: MemoryStore): MemoryHealth => {
    ...
    const avgMessagesPerSummary = store.summaries.length > 0 
    ...
    if (utilization > 90) recommendations.push("Memory nearly full - consider clearing old summaries")
    if (freshness < 30) recommendations.push("Memory is stale - new conversations will help refresh context")
    if (hallucination_risk > 70) recommendations.push("High hallucination risk - verify important responses")
    if (coherence < 40) recommendations.push("Memory fragmentation detected - auto-summarization may help")
    if (daysUntilExpiry < 7) recommendations.push("Memory expires soon - extend retention or backup important context")
  ...
  return (
    <div className={`flex items-center gap-2 ${className}`} title={`Memory Health: ${health.level}%`}>
      {/* Battery-style indicator */}
      <div className="relative flex items-center">
        {getBatteryIcon()}
        <div className="absolute inset-0 flex items-center justify-center">
          <div 
            className={`w-3 h-1 rounded-sm ${health.color === 'green' ? 'bg-green-400' : 
              health.color === 'blue' ? 'bg-blue-400' :
              health.color === 'yellow' ? 'bg-yellow-400' : 
              health.color === 'red' ? 'bg-red-400' : 'bg-gray-400'}`}
        ...
        <div className="space-y-2">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            ...
            <div>
              ...
              <span className="ml-1 font-medium">{health.indicators.hallucination_risk.toFixed(0)}%</span>
            </div>
          </div>
