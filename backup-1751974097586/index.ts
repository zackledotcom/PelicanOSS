...
const globalRateLimit = withRateLimit('global', {
  maxRequests: 100,
...
app.whenReady().then(() => {
  ...
  // Register Claude Desktop Commander handlers
  registerClaudeDcHandlers(ipcMain)

  // Register Multi-AI Orchestrator handlers
...
ipcMain.handle('chat-with-ai', async (event, data) => {
    ...
    try {
      ...
      if (memoryOptions.enabled !== false) {
        try {
